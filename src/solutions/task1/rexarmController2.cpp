...
{
	double counterClockwise = 0.160; // Can't go to that exact spot because we'd just hit the ball
	double aboveBoard = 0.038; // Trick to think the ground is a little closer than it is
	...
	//std::cout << "Hey, I picked up the ball\n";

	// Go slightly above target position and drop
	// Will have to update target form LCM here
	// but hardcoded spot is fine for now
	update_theta_2(rexarm, target);
	lcm.publish("ARM_COMMAND2", &(rexarm.goal));
	wait(rexarm, handler);
	//target.r = 0.115;
	//target.theta = 3.14159;
	...
	//std::cout << "Hey, I dropped up the ball\n";
...
{	
	...
	if(target.r > maxRange)
	{
		...
		std::cout << "new R: " << target.r << " Wrist tilt: " << wristTiltDist << "\n";
	}
