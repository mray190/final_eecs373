...
int main (int argc, char **argv)
{
	...
	while(!ballVec.empty())
	{
		if (handler.get_their_recieved())
		{
			...
			{
				pick_up_and_place(rexarm, nextBall, handler, lcm);
				ballVec.pop_back();
...
void *runLCM(void* id)
{
	...
	lcm.subscribe("ARM_COMMAND", &Handler::handleCommandList, &handler);
	lcm.subscribe("ARM_STATUS", &Handler::handleStatusList, &handler);
