...
image_u32_create_alignment (int width, int height, int alignment)
{
    ...
    im->cluster_id = (int*) calloc(1, im->height*im->stride*sizeof(int));
    im->blob_id = (int*) calloc(1, im->height*im->stride*sizeof(int));
    im->hue= (float*) calloc(1, im->height*im->stride*sizeof(float));
    im->s= (float*) calloc(1, im->height*im->stride*sizeof(float));
    im->v= (float*) calloc(1, im->height*im->stride*sizeof(float));
...
image_u32_copy (const image_u32_t *im)
{
    ...
    memcpy (out->cluster_id, im->cluster_id, im->height*im->stride*sizeof(int));
    memcpy (out->blob_id, im->blob_id, im->height*im->stride*sizeof(int));
    memcpy (out->hue, im->hue, im->height*im->stride*sizeof(float));
    memcpy (out->v, im->v, im->height*im->stride*sizeof(float));
    memcpy (out->s, im->s, im->height*im->stride*sizeof(float));
    return out;
}
