...
static void response_imu(state_t *state, sama5_response_t *response)
{
    maebot_imu_t msg;
    uint32_t msgpos = 0;
    msg.acceleration[0] = decode_s16(response->data, &msgpos, response->len);
    msg.acceleration[1] = decode_s16(response->data, &msgpos, response->len);
    msg.acceleration[2] = decode_s16(response->data, &msgpos, response->len);
    msg.angular_velocity[0] = decode_s16(response->data, &msgpos, response->len);
    msg.angular_velocity[1] = decode_s16(response->data, &msgpos, response->len);
    msg.angular_velocity[2] = decode_s16(response->data, &msgpos, response->len);
    msg.orientation[0] = decode_s64(response->data, &msgpos, response->len);
    msg.orientation[1] = decode_s64(response->data, &msgpos, response->len);
    msg.orientation[2] = decode_s64(response->data, &msgpos, response->len);
...
//            printf("%c", c);
        ...
        fread(response.data, 1, response.len, state->serial);
                          ...
                          const maebot_motor_command_t *_msg, void *_user)
{
    ...
    uint16_t left = UINT16_MAX * fabs(_msg->left_motor_speed);
    uint8_t left_rev = _msg->left_motor_speed < 0;
    uint8_t left_coast = !_msg->left_motor_enabled;

    uint16_t right = UINT16_MAX * fabs(_msg->right_motor_speed);
    uint8_t right_rev = _msg->right_motor_speed < 0;
    uint8_t right_coast = !_msg->right_motor_enabled;
