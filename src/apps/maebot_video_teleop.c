...
static int key_event (vx_event_handler_t * vh, vx_layer_t * vl, vx_key_event_t * key)
{
    ...
    if (!key->released) {
        if (key->key_code == 'w' || key->key_code == 'W' || key->key_code == VX_KEY_UP) {
            // forward
            state->cmd.left_motor_speed = MAX_FORWARD_SPEED;
            state->cmd.right_motor_speed = MAX_FORWARD_SPEED;
        } else if (key->key_code == 'a' || key->key_code == 'A' || key->key_code == VX_KEY_LEFT) {
            // turn left
            state->cmd.left_motor_speed = MAX_REVERSE_SPEED;
            state->cmd.right_motor_speed = MAX_FORWARD_SPEED;

        } else if (key->key_code == 's' || key->key_code == 'S' || key->key_code == VX_KEY_DOWN) {
            // reverse
            state->cmd.left_motor_speed = MAX_REVERSE_SPEED;
            state->cmd.right_motor_speed = MAX_REVERSE_SPEED;
        ...
        state->cmd.left_motor_speed = 0;
        state->cmd.right_motor_speed = 0;
    }

    state->cmd.left_motor_enabled = 1;
    state->cmd.right_motor_enabled = 1;
...
static void * send_cmds(void * data)
{
    ...
    while (state->running) {
        ...
        pthread_mutex_lock(&state->cmd_mutex);
        {
            ...
            maebot_motor_command_t_publish(state->lcm,  MAEBOT_MOTOR_COMMAND_CHANNEL, &state->cmd);
            printf("Current command: %f %f\n", state->cmd.left_motor_speed, state->cmd.right_motor_speed);
        }
        pthread_mutex_unlock(&state->cmd_mutex);
