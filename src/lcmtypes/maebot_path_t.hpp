...
#ifndef __maebot_path_t_hpp__
#define __maebot_path_t_hpp__
...
/**
 * maebot_path_t defines a path through the environment as a sequence of poses. The first pose in the sequence is the
...
int maebot_path_t::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int maebot_path_t::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int maebot_path_t::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t maebot_path_t::getHash()
{
    static int64_t hash = _computeHash(NULL);
    return hash;
}

const char* maebot_path_t::getTypeName()
{
    return "maebot_path_t";
}

int maebot_path_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int maebot_path_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
...
int maebot_path_t::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t maebot_path_t::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == maebot_path_t::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, (void*)maebot_path_t::getHash };
